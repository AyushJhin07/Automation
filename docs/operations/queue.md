...
Developers can temporarily bypass the durability guardrails by exporting `ENABLE_DEV_IGNORE_QUEUE=true` in conjunction with `QUEUE_DRIVER=inmemory`. This override is honored only when `NODE_ENV=development`; production builds ignore the flag and continue to fail fast. While the bypass is active the builders display an amber banner reminding you that workflow runs will be lost if the process restarts, so reconnect Redis and disable the flag as soon as you are ready to test persistence.
